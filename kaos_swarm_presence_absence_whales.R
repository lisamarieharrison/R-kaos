#presence and absence of kaos swarms linked to predator presence-absence for whales
...
for(i in 1:length(kaos_dates)) {
  ...
  pred <- read.csv("C:/Users/Lisa/Documents/phd/southern ocean/KAOS data/kaos_cetaceans.csv")  
  ...
  pred$time <- chron(times. = pred$time, format = "h:m:s")
  
  if (nrow(pred) != 0) {
    for (j in 1:nrow(pred)) {
      
      points(pred$time[j], 4, pch = "x", col = "darkgreen", cex = 3)
