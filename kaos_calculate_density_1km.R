...
setwd(dir = "C:/Users/Lisa/Documents/phd/southern ocean/KAOS/exported_integrations_1km")

file.create("C:/Users/Lisa/Documents/phd/southern ocean/KAOS/kaos_combined_density_intervals_1km.csv")

#get dates of files in folder
survey.dates <- as.numeric(substr(list.files(path = "C:/Users/Lisa/Documents/phd/southern ocean/KAOS/exported_integrations_1km", pattern = paste("(", "38kHz", ").*\\.csv$", sep = ""), full.names = F), start = 24, stop = 31))
...
for (i in 1:length(survey.dates)) {
  ...
  write.table(dat, file = "C:/Users/Lisa/Documents/phd/southern ocean/KAOS/kaos_combined_density_intervals_1km.csv", row.names = F, col.names = F, sep = ",", append = TRUE)
...
dat <- read.csv("C:/Users/Lisa/Documents/phd/southern ocean/KAOS/kaos_combined_density_intervals_1km.csv", header = F)
names(dat) <- c("time", "date", "lat", "long", "start_time", "end_time", "p", "interval_weight", "interval_length", "i")
write.csv(dat, "C:/Users/Lisa/Documents/phd/southern ocean/KAOS/kaos_combined_density_intervals_1km.csv", row.names = F)

dat <- read.csv("C:/Users/Lisa/Documents/phd/southern ocean/KAOS/kaos_combined_density_intervals_1km.csv", header = T)
dat$p[dat$p > 5000] <- NA
