...
for (i in 1:length(survey.dates)) {
  ...
  time  <- as.character(acoustic_38$Time_M[seq(1, nrow(acoustic_38), by = 50)])
  date  <- acoustic_38$Date_M[seq(1, nrow(acoustic_38), by = 50)]
  lat  <- acoustic_38$Lat_S[seq(1, nrow(acoustic_38), by = 50)]
  long  <- acoustic_38$Lon_E[seq(1, nrow(acoustic_38), by = 50)]
  start_time  <- as.character(acoustic_38$Time_S[seq(1, nrow(acoustic_38), by = 50)])
  end_time  <- as.character(acoustic_38$Time_E[seq(1, nrow(acoustic_38), by = 50)])
  ...
  mvbs  <- 10*log10(aggregate(matrix(sv, ncol = 1), by = list(rep(c(1:(length(sv)/50)), each = 50)), sum, na.rm = T)$V1/50)
  mvbs[mvbs == -Inf] <- NA
  ...
  write.table(dat, file = "C:/Users/Lisa/Documents/phd/southern ocean/KAOS data/kaos_combined_density_intervals.csv", row.names = F, sep = ",", append = TRUE)
